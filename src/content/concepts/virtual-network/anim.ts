/**
 * Small animation helpers shared by the packet and firewall simulators.
 * Everything degrades to an instant jump under prefers-reduced-motion.
 */
 ...
 * routed packet path reads as a smooth wire rather than sharp zig-zags. Each
 * interior vertex is cut back by up to `radius` along both adjacent segments and
 * bridged with a quadratic curve through the original vertex.
 */
export function roundedPathD(points: Pt[], radius = 18): string {
...
/** A short WAAPI "pop" (scale + fade in) for success/blocked markers. */
