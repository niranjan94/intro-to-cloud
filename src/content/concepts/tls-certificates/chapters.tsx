...
import { CertAnatomy } from "./cert-anatomy";
import { CertMap } from "./cert-map";
...
/**
 * The guided, chapter-by-chapter body of the TLS Certificates lesson. Chapter
 * order is fixed (anatomy, validation, where it lives, renewal, quiz); every
 * chapter's copy and interactive data comes from CONTENT[provider]. AWS and
 * Azure diverge most in the "where it lives" chapter: AWS attaches one ACM
 * certificate to an integrated service, whereas Azure has a distinct certificate
 * source per terminator with Key Vault as the shared store. The reference tabs
 * (ownership, security, then set it up) slot in ahead of the quiz.
 */
export function TlsCertificatesChapters({
  provider,
  responsibility,
  security,
  agent,
  ...
  const body = () => {
    ...
    switch (current) {
      case 0:
        return (
          <>
            <CertAnatomy content={content.anatomy} />
      ...
      case 1:
        return (
          <>
            <ValidationFlow content={content.validation} />
            {content.validation.callouts.map((c) => (
              <Callout key={c.tag} {...c} />
            ))}
          </>
        );
      case 2:
        return (
          <>
            <CertMap content={content.map} />
            {content.map.callouts.map((c) => (
              <Callout key={c.tag} {...c} />
            ))}
          </>
        );
      case 3:
        return (
          <>
            <RenewalTimeline content={content.renewal} />
            {content.renewal.callouts.map((c) => (
              <Callout key={c.tag} {...c} />
            ))}
          </>
        );
