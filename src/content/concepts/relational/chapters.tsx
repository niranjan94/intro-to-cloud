...
/**
 * The guided, chapter-by-chapter body of the Relational Database lesson.
 * Chapter order is fixed (anatomy, availability, backups, quiz); every
 * chapter's copy and interactive data comes from CONTENT[provider]. AWS and
 * Azure diverge in engine choice, high-availability shape, and backup model.
 */
export function RelationalChapters({ provider }: { provider: Provider }) {
  ...
  const body = () => {
    switch (current) {
      ...
      case 1:
        return (
          <>
            <Topology content={content.availability} />
            {content.availability.callouts.map((c) => (
              <Callout key={c.tag} {...c} />
            ))}
          </>
        );
      case 2:
        return (
          <>
            <Recovery content={content.recovery} />
            {content.recovery.callouts.map((c) => (
              <Callout key={c.tag} {...c} />
            ))}
          </>
        );
