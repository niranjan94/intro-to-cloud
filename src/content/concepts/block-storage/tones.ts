/**
 * Semantic tones for the Block Storage lesson. Several vocabularies share one
 * calm light palette: the parts of the attach picture (machine, disk), the
 * family of a volume type (ssd, hdd, top), the split between free baseline and
 * provisioned performance (baseline, provisioned), the fate of a disk when its
 * machine goes away (persist, ephemeral), and the backup timeline (snapshot).
 * Meanings are expressed as inline oklch utility classes, the same technique
 * diagram.tsx already uses.
 *
