...
import { AttachExplorer } from "./attach-explorer";
import { Callout } from "./callout";
import { CONTENT } from "./data";
import { PerformanceProvisioner } from "./performance-provisioner";
import { Quiz } from "./quiz";
import { SnapshotTimeline } from "./snapshot-timeline";
import { VolumeLadder } from "./volume-ladder";
import { VolumePicker } from "./volume-picker";

/**
 * The guided, chapter-by-chapter body of the Block Storage lesson. Chapter
 * order is fixed (attach, types, performance, snapshots, decision, quiz); every
 * chapter's copy and interactive data comes from CONTENT[provider]. AWS and
 * Azure diverge in the placement boundary (an EBS volume is pinned to one
 * Availability Zone, while Azure disks are regional and the fastest ones pin to
 * a zone), the type catalog, and the default persistence of a disk when its
 * machine is deleted.
 */
export function BlockStorageChapters({ provider }: { provider: Provider }) {
  ...
  const body = () => {
    switch (current) {
      case 0:
        return (
          <>
            <AttachExplorer content={content.attach} />
            {content.attach.callouts.map((c) => (
              <Callout key={c.tag} {...c} />
            ))}
          </>
        );
      case 1:
        return (
          <>
            <VolumeLadder content={content.types} />
            {content.types.callouts.map((c) => (
              <Callout key={c.tag} {...c} />
            ))}
          </>
        );
      case 2:
        return (
          <>
            <PerformanceProvisioner content={content.perf} />
            {content.perf.callouts.map((c) => (
              <Callout key={c.tag} {...c} />
            ))}
          </>
        );
      case 3:
        return (
          <>
            <SnapshotTimeline content={content.snapshots} />
            {content.snapshots.callouts.map((c) => (
              <Callout key={c.tag} {...c} />
            ))}
          </>
        );
      case 4:
        return (
          <>
            <VolumePicker content={content.picker} />
            {content.picker.callouts.map((c) => (
              <Callout key={c.tag} {...c} />
            ))}
          </>
        );
