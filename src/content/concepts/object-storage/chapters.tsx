...
import { AccessLayers } from "./access-layers";
import { Callout } from "./callout";
import { CONTENT } from "./data";
import { LifecycleSim } from "./lifecycle-sim";
import { NamespaceExplorer } from "./namespace-explorer";
import { Quiz } from "./quiz";
import { TierExplorer } from "./tier-explorer";

/**
 * The guided, chapter-by-chapter body of the Object Storage lesson. Chapter
 * order is fixed (namespace, tiers, access layers, access decision, lifecycle,
 * quiz); every chapter's copy and interactive data comes from CONTENT[provider].
 * AWS and Azure diverge in the namespace layering (Azure adds the storage
 * account), the tier vocabulary (storage classes vs access tiers), and above all
 * in the access model (IAM plus bucket policy plus legacy ACLs with
 * explicit-deny-wins on AWS, versus Entra RBAC plus ABAC with account-level hard
 * denies on Azure).
 */
export function ObjectStorageChapters({ provider }: { provider: Provider }) {
  ...
  const body = () => {
    switch (current) {
      case 0:
        return <NamespaceExplorer content={content.namespace} />;
      case 1:
        return (
          <>
            <TierExplorer content={content.tiers} />
            {content.tiers.callouts.map((c) => (
              <Callout key={c.tag} {...c} />
            ))}
          </>
        );
      case 2:
        return (
          <>
            <AccessLayers content={content.accessLayers} />
            {content.accessLayers.callouts.map((c) => (
              <Callout key={c.tag} {...c} />
            ))}
          </>
        );
      case 3:
        return (
          <>
            <AccessDecision content={content.accessDecision} />
            {content.accessDecision.callouts.map((c) => (
              <Callout key={c.tag} {...c} />
            ))}
          </>
        );
