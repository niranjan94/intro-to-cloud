  ...
  quiz: [
    {
      ...
      answer: 1,
      explain:
        "The namespace is flat. photos/2024/ is just a shared key prefix; the console renders a tree by splitting keys on the / delimiter. A console folder is a zero-byte object.",
    },
    {
    ...
    },
    {
      ...
      answer: 2,
      explain:
        "S3 Glacier Instant Retrieval is designed for archive data that is rarely accessed but must be retrieved immediately, at archive-level storage price with a per-GB retrieval fee.",
    },
    {
      ...
      answer: 2,
      explain:
        "S3 is designed for 99.999999999% durability of objects over a year across its storage classes. Classes differ in zone count and retrieval speed, not in designed durability.",
    },
  ],
};
  ...
  quiz: [
    {
      ...
      answer: 1,
      explain:
        "A storage account holds containers, and a container holds blobs. The storage account is the extra layer S3 has no equivalent of: it is the billing, endpoint, and redundancy boundary.",
    },
    {
      q: "Which name must be globally unique across all of Azure?",
      opts: [
        "The container name",
        "The blob name",
      ...
      answer: 2,
      explain:
        "The storage account name forms the host acmemedia.blob.core.windows.net, so it must be globally unique. Container names only need to be unique within the account.",
    },
    {
    ...
    },
    {
      ...
      opts: [
        ...
        "Chosen independently: tier is cost vs access, redundancy is where copies live",
        "Both fixed once the account is created",
        "Only available on the Hot tier",
      ],
      answer: 1,
      explain:
        "They are separate choices. The access tier (Hot, Cool, Cold, Archive) trades storage cost against access, while the redundancy option (LRS, ZRS, GRS, GZRS) sets how many copies exist and where. The one constraint is that Archive is supported only on LRS, GRS, and RA-GRS.",
    },
  ],
};
