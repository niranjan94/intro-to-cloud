...
import { ExposureTrace } from "./exposure-trace";
...
/**
 * The guided, chapter-by-chapter body of the Load Balancing lesson. Chapter
 * order is fixed (fan-out, layer, routing, exposure, choosing, quiz); every
 * chapter's copy and interactive data comes from CONTENT[provider]. AWS and
 * Azure diverge most in how the layers map to services: AWS picks the layer by
 * the load balancer type under one service (ALB is L7, NLB is L4), while Azure
 * uses two separate services (Application Gateway is L7, Azure Load Balancer is
 * L4). The reference tabs (ownership, then set it up) slot in ahead of the quiz.
 */
export function LoadBalancingChapters({
  provider,
  responsibility,
  security,
  agent,
  ...
  const body = () => {
    ...
    switch (current) {
      case 0:
        return (
          <>
            <FanOut content={content.fanOut} />
            {content.fanOut.callouts.map((c) => (
              <Callout key={c.tag} {...c} />
            ))}
          </>
        );
      case 1:
        return (
          <>
            <LayerInspector content={content.inspector} />
            {content.inspector.callouts.map((c) => (
              <Callout key={c.tag} {...c} />
            ))}
          </>
        );
      case 2:
        return (
          <>
            <ContentRouter content={content.router} />
            {content.router.callouts.map((c) => (
              <Callout key={c.tag} {...c} />
            ))}
          </>
        );
      case 3:
        return (
          <>
            <ExposureTrace content={content.exposure} />
            {content.exposure.callouts.map((c) => (
              <Callout key={c.tag} {...c} />
            ))}
          </>
        );
      case 4:
        return (
          <>
            <LayerPicker content={content.picker} />
